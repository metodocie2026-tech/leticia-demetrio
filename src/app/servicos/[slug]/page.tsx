...
async function findServico(slug: string) {
  const ofertas = await getOfertas()
  const oferta = ofertas.find((o) => o.slug === slug)
  if (!oferta || oferta.tipo !== 'servico' || !oferta.ativo) return null
  return { oferta, outras: ofertas.filter((o) => o.tipo === 'servico' && o.ativo && o.slug !== slug).slice(0, 3) }
}

export async function generateMetadata({ params }: { params: Params }): Promise<Metadata> {
  const { slug } = await params
  const found = await findServico(slug)
  if (!found) return {}

  const { oferta } = found
  const title = oferta.seoTitulo || oferta.titulo
  const description = oferta.seoDescricao || oferta.descricaoCurta
  const url = `${SITE_METADATA.url}/servicos/${oferta.slug}`
...
export default async function ServicoPage({ params }: { params: Params }) {
  const { slug } = await params
  const found = await findServico(slug)
  if (!found) notFound()
